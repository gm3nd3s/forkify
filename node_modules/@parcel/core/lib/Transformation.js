...
class Transformation {
  ...
  async run() {
    ...
    let configRequests = [...this.configs.values()].filter(config => {
      ...
      return config.includedFiles.size > 0 || config.invalidateOnFileCreate.length > 0 || config.invalidateOnOptionChange.size > 0 || config.shouldInvalidateOnStartup;
    }).map(config => ({
      id: config.id,
  ...
  async getPipelineHash(pipeline) {
    ...
    for (let transformer of pipeline.transformers) {
      ...
      if (config) {
        hash.update(await (0, _ConfigRequest().getConfigHash)(config, transformer.name, this.options));
