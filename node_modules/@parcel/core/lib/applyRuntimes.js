...
async function applyRuntimes({
  ...
  for (let runtime of runtimes) {
    ...
    }, runtime, previousDevDeps, options);
    devDepRequests.set(`${devDepRequest.moduleSpecifier}:${devDepRequest.resolveFrom}`, devDepRequest);
    await (0, _DevDepRequest().runDevDepRequest)(api, devDepRequest);
  }

  let runtimesAssetGraph = await reconcileNewRuntimes(api, connections, optionsRef);
...
async function reconcileNewRuntimes(api, connections, optionsRef) {
  ...
  return (await api.runRequest(request, {
    force: true
  })).assetGraph;
}