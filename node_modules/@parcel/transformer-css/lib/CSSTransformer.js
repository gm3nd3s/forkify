    ...
    program.walkAtRules('import', rule => {
      ...
      if ((0, _utils().isURL)(moduleSpecifier)) {
        name.value = asset.addURLDependency(moduleSpecifier, {
          loc: (0, _utils().createDependencyLocation)((0, _nullthrows().default)(rule.source.start), asset.filePath, 0, 8)
        });
      } else {
        ...
        let dep = {
          ...
          loc: (0, _utils().createDependencyLocation)((0, _nullthrows().default)(rule.source.start), moduleSpecifier, 0, 8),
          meta: {
            media
          }
        };
        ...
        parsed.walk(node => {
          if (node.type === 'function' && node.value === 'url' && node.nodes.length > 0 && !node.nodes[0].value.startsWith('#') // IE's `behavior: url(#default#VML)`
              ...
              let url = asset.addURLDependency(node.nodes[0].value, {
                loc: (0, _utils().createDependencyLocation)((0, _nullthrows().default)(decl.source.start), node.nodes[0].value)
              });
